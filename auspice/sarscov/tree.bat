...
augur filter --sequences sequences.fasta --metadata metadata.tsv  --output sequencesfiltered.fasta 
...
augur refine --tree sequencesfiltered.fasta.treefile --alignment sequencesfiltered.fasta --output-tree tree.nwk --keep-polytomies --divergence-units "mutations" --metadata metadata.tsv --timetree --clock-rate 0.0012 --output-node-data branchlengths.json --coalescent "skyline" --date-inference "marginal" --date-confidence
...
augur export v2 --tree tree.nwk --node-data branchlengths.json ntmuts.json aa_muts.json clades.json --auspice-config auspice_config.json --include-root-sequence --output cov_sarscov.json --metadata metadata.tsv


python set_lab_infections_dates.py


bonne chronologie WHO: SARS, how a global epidemic was stopped
...
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7114516/ dit que PC4-13 vient du restaurant et les autres PC4 du marché
...
BJ01-BJ04 https://www.sciencedirect.com/science/article/pii/S1672022903010234
